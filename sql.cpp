...
void dbUpdate(MYSQL* con, string column, string username, string input){
    execSQLQuery(con, "UPDATE usr\n SET " + column +  "= \"" + input + "\"\n WHERE username = \"" + username + "\";" );
}

auto dbPull(MYSQL* con, string column, string username){
    auto result = execSQLQuery(con, "SELECT " + column + " FROM usr WHERE username = \"" + username + "\";"); //\"" + username + "\";");
    MYSQL_ROW row;
        ...
        cin >> choice;
            switch(choice){
                case 1:{
                    ...
                    cin >> username;
                        if(dbPull(con, "username", username) == username){
                            cout << "User already exists.\n";
                    }
                        else if(dbPull(con, "username", username) != username){
